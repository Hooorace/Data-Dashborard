...
def cht_topFiveMonthlyTrend(data_source):
    ...
    df_monthly_history = df_views[df_views["company"].isin(company_list)][["company", "yr", "month", indicator]].groupby(["company", "yr", "month"]).sum().unstack(["yr", "month"])

    final_data = df_monthly_history.reindex(df_current_data.index)
    y_data = final_data.values
    x_data = np.vstack((np.array(pd.date_range("2017-01", "2020-06", freq = "BM"), dtype = "datetime64[M]"),) * 5)
